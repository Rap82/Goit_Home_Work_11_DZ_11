...
# Примітка : найшов помилку  в своєму попередньому коді . 
# Проблема була в лічильнику *self.current_index +=1 В конструкторі класу ))). 
# Там його не треба, бо він на момент коли прийде в метод __next_ буде вже містити максимальне значення
# і умова *if self.current_index == len(self.vectors) відразу виконається тому і поверне *None замість *result.

# Примітка : із новго два класи : *class Iterable  і *class RandomVectors:
            # Для того щоб клас був ітерований в ньому достатьно реалізувати самий простий запис ітреатора *__iter__(self)
            #     def __iter__(self):
            #         return self
            # і Реалзувати клас *__next__(self) в якому безпосередньо має за один крок оримуватись один результат який буде повертатись ітератору.

# *Примітка: Щоб гарантовано зробити ітеруємий екзепляр класу потрібно 3 речі, 
             # Перше - реалізувати метод  def __iter__(self): return self- (сам просто реалізація). 
                    #  Створює і викликає ітератор в середені екзепляра, робить його ітеруємим.(той по якому можна пройтися в циклі)
             # Друге - послідовність по якій будемо ітеруватись (списки, словники, кортежі ,тощо). Її можна або отримати як аргумент або сформувати в середені класу .
             # Третє - реалізувати метод __next__ . В ньому формуємо умовно один крок нашого циклу, 
                       # Оскільки __next__ при кожному зверненні до нього повертав один елемет  послідовності. 
                       # То  індекс після кожного разу збільшуємо на один(або зменшуємо якощо з кінця рухаємось) 
                       # щоб принаступному зверненні до __next__  повернути наступний елемент
            # Ітератори потрібні щоб и могли проходитись в циклі по полям які містять послідовності а не одне значення.(для себе)
...
class Iterable:
    ''' Клас - ітеруємий, в нашій реалізацції , Тобто  його екзепляри будуть містити поля які містять ітеруємі обєкти (спики,словник,рядки тощо) 
    а отже можна переберати їх значення в циклах *for, *while, Ітеруєме поле буде *vectors, Шаблон звернення  *Iterable(max_vectors, max_points)
      В констуркторі класу , будемо генерувати випадкові значення *x і *y, за допомогою функції *randrange(*діапазон_значень),
    Примітка : В нашому випадку діапазон від 0 до  *max_points .
      З згенерованих значень *x і *y утворюємо екзепляр класу *Vector за шаблоном *Vector(Point(x,y)) , 
      Згенерований екзепляр додаємо до нашого списку екзеплярів *self.vectors, В нашій реалізації конструктор класу виступає таким собі генератором значень ).
    Примітка : буде згенеровано рівно скільки випадкових екзеплярів яке значеннє містить аргумент *max_vectors
    Наш клас має реалізований магічний метод *__iter__(self)- (*має повертати ітератор,в нашому випадку сам себе *return self) - самої простої кострукції , 
    І метод *__next__(self)- (*ітератор) - це разом  необхідна складова що дозволяє екзеплярам класу бути ітерованими(можна проходитись по них циклами *for , *while)
    '''

    def __init__(self, max_vectors, max_points): # Конструктор класу в якому будемо формувати список з випадково згенерованих екзеплярів класу *Vector
        self.current_index = 0 # Поле лічильник який будемо використовувати для звернень до елементів списку.Початкове значення 0
        self.vectors = []      # Список в який будемо заносити згенеровані екзепляри клfсу *Vector
        for i in range(max_vectors): # Цикл в якому будемо генерувати випадкові значення *x і *y за допомогою функції *randrange(*діапазон_значень)
                                     # З цих значень, на кожному кроці циклу, формувати екзепляр класу *Vector і добавляти його внаш список . 
                                     # Примітка : наш констуркор виступить в ролі генератора значень для списку *self.vectors , 
                                                #  щоб ми могли цей клас реалізувати як ітеруємий, для інших хто буде його наслідувати чи використовувати  його екепляри. 
            x = randrange(max_points) # Генеруємо випадкове значення для *x , за допомогою функції *randrange(*діапазон_значень)
            y = randrange(max_points) # # Генеруємо випадкове значення для *y , за допомогою функції *randrange(*діапазон_значень)
            self.vectors.append(Vector(Point(x,y))) # Формуємо екзепляр класу *Vector , і добавляємо вже його до нашого списку *self.vectors 
                                # По завершенню роботи циклу *self.vectors буде містити список екзеплярів класу *Vector.
        
        print(self.vectors)  # Допоміжний принт для контрою значень  # *self.vectors ==[<__main__.Vector object at 0x0000020FE2D36000>, ..., <__main__.Vector object at 0x0000020FE2D38F50>]
        
        # *Примітка : Оскільки ми реалізували, для класу Point, магічний метод  * __str__(self) -(дивсь реалізацію в відповідному класі) 
                    # то, за допомогою f-рядка можемо в наш список занести не обєкти екзепляру *Vector, а рядки з їх "людським" виглядом
                    # Після такого запису *self.vectors.append(f"{Vector(Point(x,y))}"), стане  *self.vectors == ["Vector(9,1)", ..., Vector(3,5) ] 
                    # Де наш спискок *self.vectors буде містити вже не екзепляри класу *Vector, а їх відповідне выдображення типу рядок(*str)
                    # Тобто перетворюємо з обєкту класу в звичайний рядок(тип *str). Це так зване переворення типів , 
                    # Інколи з рядками(чи іншим типом в який перетворемо) простіше працювати ніж з екзеплярами класу.  
                    # Було *<__main__.Vector object at 0x0000020FE2D36000> - стане *Vector(1,0)- (тип *str)
                                                         # 
            
           
    def __iter__(self):
        '''Магічний метод -повертає сам об’єкт ітератора. 
        І виконує його  ініціалізацію.
        Внашому випадку повертає обєект ітератор -це екзкпляр класу *Iterable (для себе) 
        https://acode.com.ua/iterators-python/ '''
        return self
    
    def __next__(self):
        '''Магічний метод , повертає наступний елемент у послідовності. 
        Після досягнення кінця та при наступних викликах (після кінця) генерується виняток StopIteration.
    *Примітка : оскільки метод next повертає один елемент з послідовності на кожному зверненні до нього по порядку їх розміщення , 
        то він і зробить наш екзепляр ітеруємим (той по якому можна проходитись в циклі for* чи *while , 
        оскільки ці цикли, на кожному кроці(ітерації) будуть викликати цей метод __next__ атоматично , то він буде при кожному зверненні повертати там новий результат.)  
        '''
        
        if self.current_index == len(self.vectors): # Встановлюємо умову виходу з ітератора. 
                                                    # Якщо наш індек досягне довжни списку (кілкості елементів в списку ) 
                                                    # Повернути помилку *raise StopIteration 
                                                    # ця пимка вкаже нашому циклу *for чи *while припинити цикл .
                                                    # Якщо не викликати цю помилку самому то видасть Сустемну помилку (out of range) і і програма вилетить аварійно.
            raise StopIteration # викликаємо самостіно помилку *StopIteration за допоммогою оператора *raise . 
                                # Щоб припинити звернення з циклу(припинити роботу циклу який буде звертатись до нашого екзепляру)
        
        result = self.vectors[self.current_index] # Присвоюємо в *result елемент з списку *self.vectors за індексом *self.current_index
                                                   # оскільки початкове значення *self.current_index було нами встановлено 0(в конструкторі класу) то поверне перший елемент з списку *self.vectors.
        
        self.current_index += 1  # після того як в *result передали перший елемент збільшуємо наш ідекс на 1 .
                                 # Вже при наступному виклику *__next__ в резалт запишимо елемент списку за ідексом 1 (другий), 
                                 #  і так при кожному виклику *__next__  будемо повертати наступний елемент , допоки не закінчаться елементи в списку(*послідовності) 
                                 # або зовнішній цикл не престане звертатись до *__next__
        return result  # Повертаємо результат з методу .
       

class RandomVectors:
    ''' Клас - має реалізовано два обовязкові поля в констукторі: 
     self.max_vectors - якийбуде містити максимальну кількість елементів в списку . По замовчуванню всатновлено  *max_vectors=10, max_points=50
     і self.max_point - максимальне значеня координати (максимальне значення для нашого генератора випадкових значень *randrange(max_points)
      В конструкторі -  цим полям будемо передавати конкретні значення через аргументи , max_vectors=10, max_points=50 відповідно. 
      А також наш клас має реалізовано метод *__iter__(self) - ітератор в якому будемо звертатись до нашого класу *Iterable і 
      передавати значення полів *self.max_vectors, *self.max_point для того щоб клас *Iterable(self.max_vectors, self.max_point)
      Сформував і повернув нам свій екзепляр який буде мітити в полі .self.vectors список елементів по якому потім можна пройтися в циклі.
        '''
    def __init__(self, max_vectors=10, max_points=50): # Констурктор класу , приймає два обовязкові аргументи.
        self.max_vectors = max_vectors # Приствоюємо полю *self.max_vectors значення передане з першого аргументу -*max_vectors
        self.max_point = max_points   # Приствоюємо полю self.max_point значення передане з дрігого аргументу -* max_points
        
        

    def __iter__(self):
        '''Метод який буде повертати ітеруємий екзепяр класу *Iterable(self.max_vectors, self.max_point)
        З вказаними значеннями, Щоб потім проходитись по них в циклах.'''
        return Iterable(self.max_vectors, self.max_point) # Повертаємо в екзепляр класу *RandomVectors екзкпляр класу *Iterable з вказаними параметрами *self.max_vectors, *elf.max_point.

# # # +++++++++++++++++++++++++++++++++++++++++++ Тестові значення і виклик функції (не потрібний для автоперевірки.) +++++++

vectors = RandomVectors(5, 10) # Створюємо екзепляр класу *RandomVectors з конкретними параметрами/аргументами
                              # де max_vectors = 5, максимальна кількість векторів
                              #  self.max_point = 10, максимальньне значення координати яке можливе в  згенерованих векторах  

for vector in vectors:  # В цикі перевіряємо чи все в нас добре, якщо все гуд, 
    # має виводити по одному вектору в рядку в нормальному виді оскільки в нас реалізовано магічний метод *__str__  в класі *Point
                        # 
    print(vector) #  Vector(4,5)
                  #  Vector(6,1)
                  #  Vector(1,4)
                  #  Vector(1,5)
                  #  Vector(2,0)